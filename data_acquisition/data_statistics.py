...
for m in movement:
    for s in sensor:
        ...
        for window in range(number_of_windows):
            ...
            average[movement[m]][sensor[s]].append([np.mean(dataX),
                                                    np.mean(dataY),
                                                    np.mean(dataZ)])
            ...
            # print("Movement: " + m + " Sensor: " +
            #       s + " Window: " + str(window))
            # print("\tAverage: " + str(average[movement[m]][sensor[s]][window]))
            # print("\tStandard Deviation: " +
            #       str(standard_deviation[movement[m]][sensor[s]][window]))
            # print("\tKurtosis: " +
            #       str(kurtosis[movement[m]][sensor[s]][window]))
            # print("\tSkewness: " +
            #       str(skewness[movement[m]][sensor[s]][window]))
            ...
            # plt.title('PSD for' + "Movement: " + m +
            #           " Sensor: " + s + "window" + str(window))
            ...
            summary_psd_list_x = [summary_psd[movement[m]][sensor[s]][0][i]
                                  for i in sorted(list(summary_psd[movement[m]][sensor[s]][0].keys()))]
            summary_psd_list_y = [summary_psd[movement[m]][sensor[s]][1][i]
                                  for i in sorted(list(summary_psd[movement[m]][sensor[s]][1].keys()))]
            summary_psd_list_z = [summary_psd[movement[m]][sensor[s]][2][i]
                                  for i in sorted(list(summary_psd[movement[m]][sensor[s]][2].keys()))]

            if(sensor[s] != sensor["MAGNETIC_SENSOR"]):
                all_data.append(
                    [movement[m], sensor[s]] +
                    average[movement[m]][sensor[s]][window] +
        ...
        for i in range(len(summary_psd[movement[m]][sensor[s]])):
            for f in summary_psd[movement[m]][sensor[s]][i]:
                summary_psd[movement[m]][sensor[s]][i][f] = summary_psd[movement[m]
                                                                        ][sensor[s]][i][f] / number_of_windows
        ...
        #          [summary_psd[movement[m]][sensor[s]][0][k]
        #              for k in summary_psd[movement[m]][sensor[s]][0]],
        #          label="Frecuencies on x")
        # # plotting the line 2 points
        # plt.plot([k for k in summary_psd[movement[m]][sensor[s]][1]],
        #          [summary_psd[movement[m]][sensor[s]][1][k]
        #              for k in summary_psd[movement[m]][sensor[s]][1]],
        #          label="Frecuencies on y")
        # # plotting the line 3 points
        # plt.plot([k for k in summary_psd[movement[m]][sensor[s]][2]],
        #          [summary_psd[movement[m]][sensor[s]][2][k]
        #              for k in summary_psd[movement[m]][sensor[s]][2]],
        #          label="Frecuencies on z")
        # plt.xlabel('Frecuency')
        # # Set the y axis label of the current axis.
        # plt.ylabel('Average Amplitude')
...
np.savetxt('all_data_statistics.csv', all_data)
